...
def listen_print_loop(responses: object, file_path: str) -> str:
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            lastTranscriptTime = time.time()
        if time.time() - lastTranscriptTime > 5 or re.search(r"\b(exit|quit)\b", transcript, re.I):
            print("Exiting..")
            output.close()
            return transcript
...
def listen_print_loop_for_speech(responses: object, file_path: str) -> str:
    """Iterates through server responses and prints them.
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            # Switch languages if it hears a keyword:
            lastTranscriptTime = time.time()
        if time.time() - lastTranscriptTime > 5 or re.search(r"\b(arrête|stop|couette)\b", transcript, re.I):
            print("Exiting..")
            output.close()
            return transcript
...
def transcribe_audio_for_speech(language):
    ...
    config = speech.RecognitionConfig(
        encoding=speech.RecognitionConfig.AudioEncoding.LINEAR16,
        sample_rate_hertz=RATE,
        language_code=language_code,  # Specify multiple languages here
        diarization_config=diarization_config,
        alternative_language_codes=['zh-CN', 'fr-FR']  # Specify alternative languages here
    )
    ...
    with MicrophoneStream(RATE, CHUNK) as stream:  
        ...
        listen_print_loop_for_speech(responses, "/Users/prithviseran/Desktop/EZSpeech/backend/outprompts.txt")
